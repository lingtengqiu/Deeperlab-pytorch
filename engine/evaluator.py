...
class Evaluator(object):
    def __init__(self, dataset, class_num, image_mean, image_std, network,
                 multi_scales, is_flip, devices,csv_save_root =None,
        ...
        self.csv_log =None
        if csv_save_root is not "None":
            self.csv_log = csv_logger(csv_save_root)
    ...
    def run(self, model_path, model_indice, log_file, log_file_link):
        ...
        for model in models:
            ...
            result_line,_mean_iu,_mean_iu_no_back,_pixel_acc = self.multi_process_evaluation()
            if (self.csv_log!= None):
                model_name.append(model.split("/")[-1])
                mean_iu.append(_mean_iu)
                mean_iu_no_back.append(_mean_iu_no_back)
                pixel_acc.append(_pixel_acc)
        ...
        if(self.csv_log is not None):
            self.csv_log.write(zip(model_name,mean_iu,mean_iu_no_back,pixel_acc),*["epoch","mean_iu","mean_iu_no_back","pixel_acc"])
        results.close()
    ...
    def multi_process_evaluation(self):
        ...
        for d in range(nr_devices):
            ...
            #this step  : operator is very sexy
        ...
        for _ in tqdm(range(self.ndata)):
            ...
            #if verbose we compute online , we don't consider offline method
            if self.verbose:
    ...
    def sliding_eval(self, img, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            ...
            #here is operator is easily if img_scale small than crop_size
            #we add margin in img and then crop the img to img_scale and then using blinear to orignal one
    ...
    def scale_process(self, img, ori_shape, crop_size, stride_rate,
        ...
        #if long_size less than crop size we don't need slide windows
        if long_size <= crop_size:
            ...
            #stride mean slide step 2/3 size
